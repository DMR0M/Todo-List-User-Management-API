...
public class TodoServiceImpl implements TodoService {
    ...
    @Override
    @Transactional
    public TodoResponse createTodo(TodoRequest todoRequest) throws Exception {
        User currentUser = findAuthenticatedUser.getAuthenticatedUser();
    ...
    @Override
    @Transactional
    public TodoResponse toggleTodoCompletion(long id) throws Exception {
        User currentUser = findAuthenticatedUser.getAuthenticatedUser();

        Optional<Todo> completedTodo = todoRepository.findByIdAndOwner(id, currentUser);

        if (completedTodo.isEmpty()) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Todo not found");
        }

        completedTodo.get().setComplete(!completedTodo.get().isComplete());
        Todo updatedTodo = todoRepository.save(completedTodo.get());
    ...
    public void deleteTodo(long id) throws Exception {
        ...
        if (todo.isEmpty()) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Todo to be delete is not found");
        }
